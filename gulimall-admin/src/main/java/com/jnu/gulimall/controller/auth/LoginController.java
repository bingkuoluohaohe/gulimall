package com.jnu.gulimall.controller.auth;
...
import com.jnu.gulimall.controller.third.SmsSendService;
...
//        String redisValue = code+"_"+System.currentTimeMillis();
        ...
        R r = smsSendService.sendCode(phone, codeNum);
...
//            stringRedisTemplate.delete(AuthServerConstant.SMS_CODE_CACHE_PREFIX + phone);
    ...
    public String register(@Valid UserRegisterVo vos, BindingResult result, RedirectAttributes attributes) {
        ...
        if (result.hasErrors()) {
            ...
            //效验出错回到注册页面
            return "redirect:http://codekenan.icu/auth/reg.html";
        ...
        if (!StringUtils.isEmpty(redisCode)) {
            ...
            if (code.equals(redisCode.split("_")[0])) {
                ...
                vo.setUserName(vos.getUserName());
                vo.setPassword(vos.getPassword());
                vo.setPhone(vos.getPhone());
                R register = memberService.registerUser(vo);
                if (register.getCode() == 0) {
                    //成功
                    return "redirect:http://codekenan.icu/auth/login.html";
                } else {
                    //失败
    ...
    public String login(UserLoginVo vo, RedirectAttributes attributes, HttpSession session) {
        ...
        loginVo.setLoginacct(vo.getLoginacct());
        loginVo.setPassword(vo.getPassword());
        R login = memberService.loginUser(loginVo);
        if (login.getCode() == 0) {
            MemberResponseVo data = login.getData("data", new TypeReference<MemberResponseVo>() {
            });
            System.out.println(data);
            session.setAttribute(AuthServerConstant.LOGIN_USER, data);
            return "redirect:http://codekenan.icu/shop-vue/";
        } else {
            Map<String, String> errors = new HashMap<>();
